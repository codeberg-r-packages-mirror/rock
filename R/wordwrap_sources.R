...
#' @export
wordwrap_sources <- function(input,
                             output = NULL,
                             outputPrefix = "",
  ...
  for (filename in rawSourceFiles) {
    ...
    wordwrap_source(
      ...
      removeTrailingNewlines = removeTrailingNewlines,
      rlWarn = rlWarn,
      preventOverwriting = preventOverwriting,
      encoding = encoding,
      silent = silent,
      utteranceMarker = silent
    );
  ...
  if (!silent) {
    message("I just wrote ", length(rawSourceFiles), " wordwrapped sources to path '",
            output,
            "' ",
